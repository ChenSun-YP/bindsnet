...
class MelSpectrogram(object):
  ...
  def __call__(self, value):
    ...
    for i in range(len(self.window_length)):
      ...
      spec = np.asarray(torchaudio.transforms.Resize((128, self.length))(Image.fromarray(spec)))
      specs.append(spec)
...
def fetch_dataloader(pkl_dir, batch_size, num_workers, mode):
  transforms = MelSpectrogram(128, mode)
  dataset = AudioDataset(pkl_dir, transforms=transforms)
  dataloader = DataLoader(dataset, shuffle=True, batch_size=batch_size, num_workers=num_workers)
  return dataloader


print(fetch_dataloader('_sample_data/','test',10,4,'train'))
